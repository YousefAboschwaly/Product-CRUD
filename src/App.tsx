...
function App() {
  ...
  const [selected, setSelected] = useState(defaultProductObj.category);
  ...
  const openConfirmModal = useCallback(function () {
    setIsOpenConfirmModal(true);
  }, []);
  const openEditModal = useCallback(function () {
  ...
  function submitEditHandler(e: FormEvent<HTMLFormElement>) {
    ...
    if (tempColors.length > 0) errors.colors = "";
  ...
  function removeProductHandler() {
    const filtered = products.filter(
      (product) => productToEdit.id !== product.id
    );
  ...
  const renderFormInputs = formInputsList.map((input) => (
    <div className="flex flex-col" key={input.id}>
      <label
        className="mb-[2px] text-sm font-medium text-gray-500"
        htmlFor={input.id}
      >
        {input.label}
      </label>
